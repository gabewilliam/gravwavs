...
int main() {
	...
	//Used from above: a vector of binaries which can evolve via CHE for which Tmerge has been calculated. 

	double minRedshift = 0;
	double maxRedshift = 50;
	double metallicity = 0.004;
	double binaryPercent = 0.7; //percentage of stars which are in binaries.	


	//Creating a vector of redshift bins, each of width 1 and at redshifts ranging from 0-49.
	std::vector<Bin>redshiftbins; 
	
	for (int i=minRedshift; i<=maxRedshift; i++){

	Bin ithRedShiftBin(i);

	redshiftbins.push_back(ithRedShiftBin);

	}

	//performing the calculation. requires a second for-loop accross the same thing as all redshift bins must have been created before can do the rest.
	for (int i=minRedshift; i<maxRedshift; i++){

	//step one: calculate the birth rate for binaries in bin i.

		double birthRate=(redshiftbins[i].CDF(i,metallicity))*(redshiftbins[i].dMSFRdtdV(i))*binaryPercent/mTotal; //make into a function
		
		//step 2: for each binary, j, calculate the lookback time at the point of merging. 

		for (int j=0; i<Nr; j++){

			double desiredMergeLookbackTime=(redshiftbins[i].gettLookback())-binaries[j].gettMerge(); //check that units are consistent. 
			
			//step 3: if the binary has merged, loop through all the redshift bins with tLookback less than that of bin i, to find the first one where the lookback time is 			greater than the desired look back time. The birth rate is then added to the sum of the merge rates in the bin before that with the greater lookback time than 				desired.  
	
			if (desiredMergeLookbackTime>=0){

				for (int k=0; k<i; k++){
					
					if (desiredMergeLookbackTime>redshiftbins[k].gettLookback()){

						redshiftbins[k-1].addToMergeRateSum(birthRate);
	...
	for (int i=minRedshift; i<maxRedshift; i++){
		...
		double mergeRate=(redshiftbins[i].getMergeRateSum())*(redshiftbins[i].getCounter());

		std::cout<<"For redshift bin "<<i<<" the merge rate is "<<mergeRate<<std::endl;

	}
	
	//not sure where this file should close!
	fclose(popFile);
