...
struct parameters{
	...
	// Maximum frequency to investigate
	double maxFreq;
...
void setFundamentalParameters(
	...
	// Set constant redshift
	//params->redShift = rs(dist);
	...
	// Convert masses into units of distance and apply redshift
	double m1 = m_1*convFact*SM_CONST*(1.0 + params->redShift);
	double m2 = m_2*convFact*SM_CONST*(1.0 + params->redShift);
	...
	// Set maximum frequency based on number of data points
	double nPoints = pow(2.0, power);
	
	params->maxFreq = (nPoints - 5.0)*params->df/4.0;
...
int gravitationalWave(parameters *params, vector<Signal> *sigs){
	...
	double nPosFreq = params->maxFreq/params->df + 1.0;
	...
	// Initialise frequency and both wave components
	double freq = 0.0;
	double hplus = 0.0;
	double hcross = 0.0;
	...
	for (int i = 0; i < nPosFreq; i++){
		...
		// Output the amplitude in units Hz^-1/2
		gw_Hz = gravWav/C_CONST;
		hplus = gw_Hz.real();
		hcross = gw_Hz.imag();
		
		gw_Hz.real(hplus*inclinationEffects(params, Plus));
		gw_Hz.imag(hcross*inclinationEffects(params, Cross));
