...
int main(){
	...
	double M_min, M_max, Dist, nSteps, totTime, fMin;
	// Mass 1
	...
	cout << endl << "Please enter the total duration of each data sample (in Seconds): ";
	cin >> totTime;
	// Minimum detector sensitivity
	cout << endl << "Please enter the minimum detector sensitivity (in Hz): ";
	cin >> fMin;
	cout << endl;
	...
	// Safety checking
	if (M_min == 0.0 || M_max == 0.0 || Dist == 0.0 || M_min >= M_max || totTime == 0.0){
		cout << "Cannot compute" << endl;
		return FAILURE;
	}
	
	// Set time of arrival of signal
	double initTime = totTime;
	...
	// Set power to which to raise 2 to determine number of data pointer
	double pow = 20.0;
