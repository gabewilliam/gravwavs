...
void inclinationEffects(parameters *params, Signal *wave){
	
	int sz = wave->waveform[0].size();
	...
	for(int i=0; i < sz; i+=2){
		...
		//h+ is real, hx is img
		wave->waveform[1][i] *= fplus;
		wave->waveform[1][i+1] *= fcross;
...
int gravitationalWave(parameters *params, vector<Signal> *sigs){
	...
	Signal * sigPtr = &totSig;
	inclinationEffects(params, sigPtr);
