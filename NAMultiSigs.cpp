...
void binaryInspiral(double mA_z, 
	...
	while (tMerge > 0.0){	
		...
		signalVect.waveform[0].push_back(tCurrent);
		signalVect.waveform[1].push_back(waveForm);				 		
		
	}
	
	sig->push_back(signalVect);
}
...
int main(){
	...
	double maxM = 200.0;
	double M1, M2;	
	// Randomly set luminosity distance in MPc
	// NEED TO MAKE THIS RANGE REALISTIC
	double maxDist = 1000.0;
	double lumDist = double(rand())*maxDist/double(RAND_MAX);	
	...
	for(int i = 0; i<nSigs; i++){
		...
		binaryInspiral(M1, M2, lumDist, lowBound, m);
		cout << "Mass 1: " << M1 << ", Mass 2: " << M2 << endl;
	}
	
	if(saveSignals("MultiSignal.csv", m, csv)){
