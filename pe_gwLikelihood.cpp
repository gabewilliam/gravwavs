...
double PdhFunction( double m1, double m2, std::string signalFile ){
	...
	loadSignals(signalFile, idt, csv); //Loads signal into dt
...
vec_d ParameterFunction( double m1, double m2, vec_d t ){ 
	...
	for( int i = 0; i < n; i++ ){
		ht.push_back( generate ); //PLACEHOLDER: INSERT PARAMETER FUNCTION IN PARENTHESIS
	}
...
vec_d NoiseFunction( vec_d f ){
	...
	for( int i = 0; i < n; i++ ){
		sf.push_back( 1.0 ); //PLACEHOLDER: INSERT NOISE FUNCTION IN PARENTHESIS
	}
