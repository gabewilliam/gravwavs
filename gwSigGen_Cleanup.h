...
double computeTransitionComps(double a, 
							  double b, 
							  double c, 
							  Parameters *P){
...
int simulateGravitationalWave(Parameters *P, 
	...
	for (int p=0; p < int(nPoints); p++){
		...
		complexSignal.waveform[0].push_back(0.0);
		complexSignal.waveform[1].push_back(0.0);
...
void setParameters(double M1,
	...
	double massConversionFactor = G_CONST/pow(C_CONST, 2.0);
	...
	P->fSymmetricMass = (M1*M2)/pow(P->fTotalMass, 2.0);
	...
	computePhaseComps(1.7516E-1, 7.9483E-2, -7.2390E-2, 0., P);
	P->fPhaseExpansions[1] = 0.0;
	P->fPhaseExpansions[2] = 
	computePhaseComps(-5.1571E1, -1.7595E1, 1.3253E1, 2., P);
	P->fPhaseExpansions[3] = 
	computePhaseComps(6.5866E2, 1.7803E2, -1.5972E2, 3., P);
	P->fPhaseExpansions[4] = 
	computePhaseComps(-3.9031E3, -7.7493E2, 8.8195E2, 4., P);
	P->fPhaseExpansions[5] = 0.0;
	P->fPhaseExpansions[6] = 
	computePhaseComps(-2.4874E4, -1.4892E3, 4.4588E3, 6., P);
	P->fPhaseExpansions[7] = 
	computePhaseComps(2.5196E4, 3.3970E2, -3.9573E3, 7., P);
	
	//-- Transitional frequencies and sigma
	P->fMerge = 
	computeTransitionComps(2.9740E-1, 4.4810E-2, 9.5560E-2, P);
	P->fRing = 
	computeTransitionComps(5.9411E-1, 8.9794E-2, 1.9111E-1, P);
	P->fSigma = 
	computeTransitionComps(5.0801E-1, 7.7515E-2, 2.2369E-2, P);
	P->fCutoff = 
	computeTransitionComps(8.4845E-1, 1.2848E-1, 2.7299E-1, P;
