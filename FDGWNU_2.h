...
void setFundamentalParameters(
	...
	params->psi[3] = computePhaseComps(6.5866E2, 7.9483E-2, -7.2390E-2, n, M, 3.);
...
int gravitationalWave(parameters *params, vector<Signal> *sigs, int nData){
	...
	for (int i = 0; i < nData; i++){
		...
		reSig.waveform[0].push_back(freq_Hz);
		
		totSig.waveform[0].push_back(-freq_Hz);
		
		imSig.waveform[0].push_back(-freq_Hz);
		
		totSig.waveform[1].push_back(gravWav.real());
		reSig.waveform[1].push_back(gravWav.real());
		
		totSig.waveform[1].push_back(gravWav.imag());
		reSig.waveform[1].push_back(gravWav.imag());
