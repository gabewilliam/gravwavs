...
int main() {
	...
	/ on its logarithmic distribution. All of the generated systems are 
	/ output to one file. Those satisfying the 20 < m < 300 component
	/ mass limits are output to a second file. In the output files, 
	/ the masses are given in solar masses, radii in solar radius and 
	/ separation in AU.*/
	for(int n = 0; n < N; n++) {
		...
		fprintf(popFile1,"%.15g,%.15g,%.15g,%.15g,%.15g\n",m1,m2,r1,r2,a);

		if((m1 >= mMin) && (m2 >= mMin)
			&& (m1 <= mMax) && (m2 <= mMax)) {

			fprintf(popFile2,"%.15g,%.15g,%.15g,%.15g,%.15g\n",m1,m2,r1,r2,a);
